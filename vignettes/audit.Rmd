...
After either kind of run, disk holds the **full** portfolio: `write_registry_audit_record()` merges this run into `audit_jobs.csv`, then rebuilds JSON and RDS from every CSV row.

### Seed without live runs

`seed_registry_audit_jobs()` fills gaps so the health bar is not empty before the first full audit. It does **not** execute replications.

- From a prior `audit_latest.rds` (`from_rds = TRUE`)
- From bake timings / local artifact mtimes as provisional ok rows (`from_bake = TRUE`)
- Existing `source = "audit"` rows are kept; bake/artifact rows only fill gaps (or refresh provisional bake rows)
...
`refresh_registry_audit_summary()` rewrites `audit_summary.json` and `audit_latest.rds` from the current CSV without running audits. Use after manual CSV edits or after seeding.
