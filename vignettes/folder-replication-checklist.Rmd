...
root `replication.yml` via `register_study()` (step 4) — nothing is
...
- `paper.doi` — full DOI URL (or `paper.study_handle` for unpublished local scaffolds)
- `paper.title`
- `paper.source_repository` — URL of the original data / materials deposit; **required** for DOI / registry-bound studies. Handle-only local scaffolds ([get_started()]) may omit it; [check_and_bake_study()] warns (PASS with reminder) until registry registration
...
- `repo` or `paper.study_repo` — GitHub slug (`org/repo`); inferred from folder name when it starts with `rep-`. Optional for unpublished local folders until you publish
...
`incomplete: true` excludes the step from baking and from `audit_everything()`
(neither success nor failure). Prefer structured `requires_engine:` /
`data_unavailable:` so Shiny can show the right icon and partial-replication
popup. Put a step in `steps:` only if it is a **replication claim**; blocked
prep off the claim path stays in README / popup — not as orphan Unavailable
nodes. Recover DAGs at **wrapper granularity** from author README tables —
especially OpenICPSR AER packages. See [Contributing principles](contributing-principles.html).
...
Before running the full checklist, a quick **manual smoke check** from the
study repo root confirms the study resolves and runs without any registry
setup — pass `"local"` (the working-directory study) instead of a DOI:
...
This runs the contributor checklist against your study root `replication.yml`
(via [check_and_bake_study()], which uses an internal checker).
...
This validates the study, builds a lightweight stub from the study repo's root
`replication.yml` (`maintainer`, `collections`, `languages`, and repo
pointers — never the full `steps:` list), writes it to
`registry/studies/<folder>.yml`, and rebuilds `index.csv`.

After registering several studies, light-refresh derived files (and optionally
run a live audit):
...
`check_glm_table_benchmark()` is injected when the package loads substantive check scripts (via [audit_everything()] or [check_and_bake_study()] with `full_replication = TRUE`). Study-specific benchmark values belong in the study repo, not the package.

Call the same function from `tests/testthat/test-<step_id>.R` so `testthat::test_dir()` exercises it locally.

**Maintainers:** [check_and_bake_study()] with `full_replication = TRUE` runs defined substantive checks. [audit_everything()] runs them registry-wide by default (`substantive = TRUE`); failures appear as `[substantive]` in the audit summary.

### Maintainer shortcut

```{r, eval = FALSE}
options(replicateEverything.registry_root = "../registry")
register_study("../rep-10.1177-00491241211036161", audit = TRUE)
```
...
For studies maintained as R packages, see `vignette("package-replication-checklist", package = "replicateEverything")` and [check_and_bake_study()].
