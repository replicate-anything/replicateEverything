...
| **Package-backed** | `papers/<folder>/replication.yml` stub only | Study R package on GitHub |
...
`add_paper()` writes `papers/<doi-folder>/replication.yml` and updates `index.csv`.
