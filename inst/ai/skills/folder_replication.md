...
| `blocked_reason` | Human text for Shiny hover / tooltips on `incomplete:` / engine / data gaps |
| `shiny_run` | `false` to disable **Shiny Live Run only** (greyed Run; fixed label `[live run not available on shiny]`). Display / Code / package `run_replication()` / bake / audit are unchanged. Prefer this over `incomplete:` when the step still runs from R. Default `true` (omit when runnable). |
...
| Shiny Live Run off (step still package-runnable) | `shiny_run: false` | Greyed **Run** + `[live run not available on shiny]`; Display + Code unchanged | **Runs** (not a gap) |
