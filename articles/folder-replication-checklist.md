...
This runs every replication, saves PNG/HTML (or RDS) under `artifacts/`,
...
- call `run_replication(..., format = TRUE)` when `format:` is set in
  yaml
- compare formatted output to the committed artifact (HTML normalize or
  PNG md5)
...
- `registry/replication.yml` — copy to `registry/studies/<folder>.yml`
...
**Option B — manual:** copy the two files from `registry/` in the study
