...
### Substantive (published-value) tests

Beyond smoke tests (`run_replication` returns an object), studies should
verify **published numbers** where possible. Add
`tests/substantive/<step_id>.R` defining
`substantive_check_<step_id>(object)`:
...
Call the same check from `tests/testthat/test-<step_id>.R`.
\[audit_everything()\] runs substantive checks automatically when the
script exists (`substantive = TRUE`, default).
