...
- `registry/index.csv` — one row with `collections`, `maintainer_name`,
  `maintainer_email`, `languages`, and `article_url` when set

Commit these files with your study PR. A registry **maintainer**
installs them in the central registry (step 4).
...
This copies `registry/replication.yml` from the study repo to
...
**Manual alternative:** copy `registry/replication.yml` to
`registry/studies/<folder>.yml`, then:
