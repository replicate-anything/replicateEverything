test_that("build_shiny_studies_cache writes schema with notes and related", {
  ...
  expect_equal(cache$schema_version, 2L)
  expect_true(length(cache$studies) == 2L)
  expect_true(is.list(cache$ui))
  expect_true(length(cache$ui$select_choices) >= 2L)
  expect_true(length(cache$ui$collection_choices) >= 1L)
  select_ch <- shiny_studies_select_choices(cache)
  expect_true(length(select_ch) >= 2L)
  expect_true("10.9999/example" %in% unname(select_ch))
  coll_ch <- shiny_studies_collection_choices(cache)
  expect_true("__all_studies__" %in% unname(coll_ch))
  expect_true("APSR" %in% unname(coll_ch))
  expect_true("IPI" %in% unname(coll_ch))
