...
test_that("code_setup_box_content describes Blair Stata table setup", {
  ...
  expect_match(content$one_liner, "run_replication")
  expect_match(content$step3, "run_replication")
  expect_match(content$step3, "do \"")
  expect_true(grepl("not eval\\(parse", content$step3) || grepl("do not eval", content$step3, ignore.case = TRUE))
