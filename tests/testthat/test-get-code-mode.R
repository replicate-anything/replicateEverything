test_that("get_code emits R figure usage tip with yaml-implied call", {
  with_fixture_opts({
    ...
    expect_true(grepl("To produce the figure:", tip, fixed = TRUE))
    expect_true(grepl("working directory to the study repository root", tip, fixed = TRUE))
    expect_false(grepl("Prefer run_replication", tip, fixed = TRUE))
...
test_that("get_code emits R table usage tip", {
  with_fixture_opts({
    ...
    tip <- paste(msgs, collapse = "\n")
    expect_true(grepl("R function definitions", tip, fixed = TRUE))
    expect_true(grepl("To produce the table:", tip, fixed = TRUE))
    expect_true(grepl("working directory to the study repository root", tip, fixed = TRUE))
    expect_false(grepl("Prefer run_replication", tip, fixed = TRUE))
    expect_true(grepl("yaml-implied", tip, fixed = TRUE))
    expect_true(grepl("eval\\(parse\\(text = get_code", tip))
  })
})
...
test_that("get_code emits Stata usage tip for stata fixture", {
  with_fixture_stata_opts({
    ...
    expect_false(grepl("eval\\(parse", tip))
    expect_false(grepl("mode = \"run\"", tip, fixed = TRUE))
    # WD noted once — not repeated on each bullet
    expect_equal(length(gregexpr("working directory", tip, fixed = TRUE)[[1]]), 1L)
  })
})
...
test_that("emit_get_code_usage_message is engine- and type-aware", {
  ...
  expect_true(grepl("To produce the step:", msgs[[1]], fixed = TRUE))
  expect_false(grepl("Prefer run_replication", msgs[[1]], fixed = TRUE))
  expect_true(grepl("Python script text", msgs[[2]], fixed = TRUE))
  expect_true(grepl("To produce the figure:", msgs[[2]], fixed = TRUE))
  expect_false(grepl("Prefer run_replication", msgs[[2]], fixed = TRUE))
  expect_false(grepl("eval\\(parse", msgs[[2]]))
})
